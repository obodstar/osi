...
def Menu(PathFFMPEG:str, PAthcookie:str, checkingfolders:str, waktu_upload=20, waktu_jeda=0):
    ...
    for idx, (folder_akun, cookie) in enumerate(ListDAta, start=1):
        video_folder = os.path.join(folder_akun, 'video_upload')
        videos = OpenFolder(video_folder)
    ...
    while masih_ada:
        ...
        for akun in akun_data:
            ...
            idx = akun["idx"]
            folder_akun = akun["folder"]
            cookie = akun["cookie"]
            video = akun["videos"].pop(0)  # 🔑 INI KUNCINYA

            print(f'[•] Upload Selang-seling | Akun {idx} | {os.path.basename(video)}\n')

            # ====== KODE UPLOAD LAMA KAMU (TIDAK BERUBAH) ======

            with open(f'{folder_akun}\\log.txt', 'r', encoding='utf-8') as f:
                pathlog = f.read().splitlines()

            basefilename = os.path.basename(video)
            audio_asset_id = basefilename.split('.')[0].split('_')[1]

            if basefilename in pathlog:
                print('[✓] Sudah pernah upload, skip\n')
                continue
            ...
            if not filemusic:
                print('[✗] Audio tidak ditemukan, skip\n')
                continue
            ...
            caption = f"{random.choice(basecaption)}\n\n{random.choice(basehashtag)}"
...
if __name__ == '__main__':
    ...
    if PathFFMPEG is None:
        ...
        exit()
    try:
        PAthcookie = input('[+] File Data Akun (cookie.txt) : ')
